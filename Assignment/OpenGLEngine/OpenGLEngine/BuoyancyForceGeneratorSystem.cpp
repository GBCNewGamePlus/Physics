...
namespace Reality
{
	...
	void BuoyancyForceGeneratorSystem::Update(float deltaTime)
	{
		for (auto e : getEntities())
		{
			...
			// Verifies self-destroy - if this is past the point of no return
			//if (sphereTransform.position.y < 1.5 * buoyancy.maxY - sphereTransform.scale.y) {
				//getWorld().destroyEntity(e);
			//}
